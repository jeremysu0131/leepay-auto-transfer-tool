...
class Worker extends VuexModule implements IWorkerState {
  ...
  public worker = {} as WorkerFactory;

  @Mutation
  SET_WORKER(worker: WorkerFactory) {
    this.worker = worker;
  }
  @Mutation
  ...
  public async SetWorker(taskDetail: TaskDetailModel) {
    try {
      ...
      var workerFactory = new WorkerFactory(taskDetail);
      console.log("worker factory", workerFactory);
      this.SET_WORKER(workerFactory);
  ...
  async UnsetWorker() {
    ...
    this.SET_WORKER({} as WorkerFactory);
